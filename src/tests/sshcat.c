...
int main(int argc, char *argv[])
{
    ...
    const char *commandline = "cat >/tmp/output.txt";
    const char *username    = "rnd";
    const char *password    = "hopsasa";
...
*/
	char * id_rsa_pub = "/home/rnd/.ssh/id_rsa.pub"; char * id_rsa = "/home/rnd/.ssh/id_rsa"; 
    while ((rc = libssh2_userauth_publickey_fromfile(session, username, id_rsa_pub, id_rsa, password)) == LIBSSH2_ERROR_EAGAIN);
	if (rc) {
    ...
    while( (rc = libssh2_channel_exec(channel, commandline)) == LIBSSH2_ERROR_EAGAIN ) waitsocket(sock, session);
    if( rc != 0 )
	{
    ...
    else
	{
        ...
        int running = 1;
        int bufsize = BUFSIZE;
        char *buffer = "dette er en test dette er en test dette er en test dette er en test dette er en test dette er en test dette er en test ";
        int totsize = sizeof(buffer); // 1500000;
        int totwritten = 0;
        int totread = 0;
        int partials = 0;
        int rereads = 0;
