    ...
    foreach my $host (@hostlist)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                ## avoid problems with motd etc and faileure when process not running
                #my $cmd = "ps -C exabgp -o pid= >/dev/null && stat -c%X /proc/`ps -C exabgp -o pid=|tr -d ' '`";
                ...
                if ($result eq '') {
                    logit("$host: service exabgp down command output empty");
                    $ok_to_connect = 0;
                    #
