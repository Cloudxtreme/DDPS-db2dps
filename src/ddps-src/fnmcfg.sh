...
function updatecfg()
{
    ...
    local TMPDIR=`mktemp -p ${WORKDIR} -d XXXXXXXXXX.updatecfg`
...
{
    test -d ${WORKDIR} || mkdir ${WORKDIR}
    local TMPDIR=`mktemp -p ${WORKDIR} -d XXXXXXXXXX.edit`
    local OLDDIR=`pwd`
    cd ${TMPDIR}
...
{
    test -d ${WORKDIR} || mkdir ${WORKDIR}
    local TMPDIR=`mktemp -p ${WORKDIR} -d XXXXXXXXXX.loop`
    cd ${TMPDIR}
...
# Other services? pgpool, postgres ww1/ww2 alive? Permissions??
function status()
{
...
.yellow { background: #FFF8C6; color: #00496B; }
</style>
</head>
<body style="font: normal 12px/150% Arial, Helvetica, sans-serif; background: #fff;">
<div class="unistyle">
...
(
case ${AERRORS} in
    0)  cat ${ANNOUNCEMENTS_STATUS_FILE}
        ;;
    *)  echo "<span class="yellow">"
        cat ${ANNOUNCEMENTS_STATUS_FILE}
        echo "</span>"
        ;;
esac
) >> ${REPORT_FILE}
...
EOF
    ...
    logit "report done - status=$ERRORS"
}
...
function db2cfg()
{
    ...
    test -d ${WORKDIR} || mkdir ${WORKDIR}
    local TMPDIR=`mktemp -p ${WORKDIR} -d XXXXXXXXXX.db2cfg`
    logit "tmpdir: ${TMPDIR}"
    local OLDDIR=`pwd`
    cd ${TMPDIR}
