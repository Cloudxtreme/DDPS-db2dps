...
#   - host has 2 or more interfaces; one is for the Internet / VPN access and the other
#     will be connected to a mirror port while having no address.
#   - Internet interface is connected and is not the fastest interface. Address
#     assignment is defined in preseed.cfg: default use dhcp, fallback to specific 
#     address (must be set correct!)
#   - fastest interface or next similar will be used as fastnetmon listening interface
#
...
### Build 10Gb or 1Gb drivers and load in /etc/rc.local

Running the function make_rc_local which will build the 10Gb and 1Gb drivers, disable
IPv6 in /etc/sysctl.conf and load the driver(s) via /etc/rc.local.
...
EOF
	...
	echo '  5.  Disable IPv6 in `/etc/sysctl.conf`'
...
EOF
	...
	echo '  6. If the drivers are not allready in `/opt/netmap/ kernel version`'
...
### Compile and install drivers modified by Pavel Odintsov
...
function main()
{
    # check on how to suppress newline (found in an Oracle installation scrip   t ca 1992)
    echo="/bin/echo"
    case ${N}$C in
    ...
    # Process argument:
    #   Default '' -- do-it-all
    #   Or -r reconfig with -i inet interface and -f fastnetmon interface
    #   Rebuild IP configuration and fastnetmon etc. config and select igb or ixgbe driver (write to /etc/rc.local)
    #   -r
    #   -i interface:cidr:gw
    #   -f interface
    #   Or
    #   -u
    #   update drivers if required by kernal update
    #
    DO="do_it_all"

    while getopts vhri:fu opt
    do
    case $opt in
        v)  VERBOSE=TRUE
        ;;
        r)  DO="reconfig"
        ;;
	i)  interface_info=$OPTARG
	;;
	f)  fastnetmon_info=$OPTARG
	;;
        h)  echo "usage: $0 [-v]"
            exit
        ;;
    ...
    do_it_all)
	    sleeptime=2
	    echo "This will re-install the latest igb ang ixbge and wipe /etc/rc.local"
	    echo "Shure you want to do this? (sleep $sleeptime seconds) ... "
	    sleep $sleeptime
