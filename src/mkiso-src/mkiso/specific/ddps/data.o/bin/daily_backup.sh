...
The backup of ${HOSTNAME}.${DOMAINNAME} failed. See logfile below.

EOF

	cat ${MY_LOGFILE}

) | mailx -s "Backup failed on ${HOSTNAME}.${DOMAINNAME}" ${RCPT:="fwsupport@uni-c.dk"}
	logger -p mail.crit "backup FAILED"
	;;
esac
...
#	$State: Exp $
# HISTORY
#	See \fCrlog\fR $Id: daily_backup.sh,v 1.1 2016/04/09 09:02:42 root Exp $
# AUTHOR(S)
